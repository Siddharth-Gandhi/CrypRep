...
class DES:
    ...
    def run(self, text, password, action):
        ...
        for text_bits in self.__split_array(text_bit_array, 64):
            ...
            for i in range(16):
                ...
                if action == 'encrypt':
                    # If encrypt use Ki from first key...
                    temp = self.__xor(self.keys[i], right_expanded)
                else:
                    # ... if decrypt use Ki from last key
                    temp = self.__xor(self.keys[15 - i], right_expanded)
                # Method that will apply the S-Boxes
                temp = self.__substitute(temp)
                # Perform permutation after S-Box
            ...
            final_permutation = self.__permutation(
                right + left, FINAL_PERMUTATION)
    ...
    def __create_bit_array(string):
        ...
        if bit_string_len > 64:
            ...
            new_len = 64 * (int(bit_string_len / 64) +
                            int(bit_string_len % 64 != 0))
    ...
    def bin_2_formatted_hex(bit_array, fixed_length=True, length=16):
        ...
        :param fixed_length: Sets if length needs to be fixed should be False when result is printed to match the
        original string
...
def run_des(msg, password, action):
    ...
    des_run['Keys'] = [des.bin_2_formatted_hex(
        des_key, length=12) for des_key in des.keys]
